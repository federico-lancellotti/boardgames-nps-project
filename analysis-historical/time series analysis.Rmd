...
for (i in 5:40) {
  df=rated[rated[,i]==0,89:ncol(rated)]
  df.dec=decompose(ts(transpose(df),frequency = 52, 
                      start = c(2016, 42)))
  median.trend_cat[i,] <- apply(df.dec$trend, 1, median)
  #median.season[i,] <- apply(as.matrix(df.dec$seasonal,
                                  #nrow=nrow(df),ncol=370), 1, median)
  }
for (i in 41:88) {
...
median.trend_cat=median.trend_cat[5:nrow(median.trend_cat),]
saveRDS(median.trend_cat,file="C:/Users/Silvia/Desktop/23-24/NONPAR/project/median_trend_cat.RDS")
...
plot_medians <- function(col_ind) {
  folder <- "plot_ts/medians time series"
  ylabel <- "Ratings"
  
  median.0 <- median.trend_cat[col_ind,]
  ...
  p <- ggplot(data=median.df, aes(x = x)) +
    geom_line(aes(y = y0, color = "0"), linetype = "solid", linewidth = 0.5) +
    geom_line(aes(y = y1, color = "1"), linetype = "solid", linewidth = 0.5) +
