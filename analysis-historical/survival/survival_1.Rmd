...
result = permuted_logrank_test(data$Economic_fact, data$time, data$status, iter = 1000)
...
#4.2.A)Weight: !!!!!!!
weight_quadratic = poly(data$weight, degree = 2) #no raw
cox.weight = coxph(Surv(time, status) ~ weight_quadratic, data = data)
cox.weight
summary(cox.weight)
...
# ##4.3.A)Weight:
# cox_weight=coxph(Surv(time, status) ~ bs(weight, df=8), data = data)
# summary(cox_weight)
# #Plot (baseline survival function S0(t)):
...
# # Per residui normali:
# # #Ftest:
# # anova(cox.weight,cox_weight, test = "F")  #pval < 2.2e-16 ***
```
